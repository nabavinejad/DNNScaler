...
N = 20
latencyReading = [-1] * N
nth_percentile = 95  # 95% tail latency
...
class CountdownTask:
    ...
    def run(self):
        while self._running:
            os.system('nvidia-smi --query-gpu=timestamp,utilization.gpu,utilization.memory,power.draw,clocks.sm --format=csv ,noheader,nounits, -f "InceptionV1_nvidiasmi_output.csv"')
            time.sleep(1)
...
def getResnet50():
  with gfile.FastGFile("frozen_resnet_v2_152.pb",'rb') as f:
    graph_def = tf.GraphDef()
    graph_def.ParseFromString(f.read())
  return graph_def


def timeGraph(gdef,batch_size=128,image_folder='images',latencyMS = 30, powerCap = 50, result_file = "ResultsLog.txt"):
...
##    dummy_input = np.random.random_sample((batch_size,224,224,3))
  ...
  with g.as_default():
    ...
    out = tf.import_graph_def(
      ...
      return_elements=[ "resnet_v2_152/predictions/Softmax"]
    )
    out = out[0].outputs[0]
    outlist.append(out)
  ...
  with tf.Session(graph=g,config=tf.ConfigProto(gpu_options=gpu_options)) as sess:
      ...
      listBatchSizeSteps = [x for x in range(1,128)]
      ...
      resultsFile.write("time stamp, Througput (image/sec), batch size, latency (ms), latency constraint (ms)\n")
      ...
      while True:
          ...
          if (time.time() - two_second_start) > 3 :
              ...
              #latecnyTailNth = np.percentile(latencyReading,nth_percentile)
              latecnyTailNth = np.mean(latencyReading)

              #if power cap is meeting, do not do anything. we use 0.9 as a margin to avoid lots of fluctuations
              if latecnyTailNth <= latencyMS and latecnyTailNth >=  (0.85 * latencyMS):
                  #powerReading[-N:-1] = [0] * (len(powerReading) - 20)

                  latencyReading[:] = []
                  pass

              elif latecnyTailNth < (0.85 * latencyMS):   # Power less than power cap
                  latencyReading[:] = []
                  if BestBS == MAXINDEX:
                      pass
              ...
              elif latecnyTailNth > latencyMS:          # Power More than Power Cap
                  latencyReading[:] = []
                  if BestBS == MININDEX:
                      pass
          ...
          latencyReading.append(timings[-1] * 1000)
          ...
          resultsFile.write(
              str(time.time()) + "," + str(ThroughputPerSecond[-1]) + "," + str(newbatchSize) +
              "," + str(int(timings[-1] * 1000)) + "," + str(latencyMS) + "\n")
...
if "__main__" in __name__:
  ...
  if f.native:
    ...
    if os.path.exists('runtimes_ResNetV2_152_BatchScaler.txt'):
        append_write = 'a'  # append if already exists
    else:
        append_write = 'w'  # make a new file if not

    runtimeResults = open('runtimes_ResNetV2_152_BatchScaler.txt', append_write)
    runtimeResults.write('Total Execution Time, ' + 'BS  = ' + str(f.batch_size) + ', ' +str(endTime - startTime))
