...
def getResnet50():
  with gfile.FastGFile("frozen_resnet_v2_50.pb",'rb') as f:
...
##    dummy_input = np.random.random_sample((batch_size,224,224,3))
  ...
  with g.as_default():
    ...
    out = tf.import_graph_def(
      ...
      return_elements=[ "resnet_v2_50/predictions/Softmax"]
    )
    out = out[0].outputs[0]
    outlist.append(out)
...
if "__main__" in __name__:
  ...
  if f.native:
    ...
    if os.path.exists('runtimes_ResNetV2_50_BatchScaler.txt'):
        append_write = 'a'  # append if already exists
    else:
        append_write = 'w'  # make a new file if not

    runtimeResults = open('runtimes_ResNetV2_50_BatchScaler.txt', append_write)
