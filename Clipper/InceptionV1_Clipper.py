  ...
  with tf.Session(graph=g,config=tf.ConfigProto(gpu_options=gpu_options)) as sess:
      ...
      while True:
          ...
          if (time.time() - two_second_start) > 3 :
              ...
              elif latecnyTailNth < (0.85 * latencyMS):   # latency less than latency cap
                  latencyReading[:] = []
                  if BestBS == MAXINDEX:
                      pass
                  else:
...
if "__main__" in __name__:
  ...
  if f.native:
    ...
    if os.path.exists('runtimes_InceptionV1_Clipper.txt'):
        append_write = 'a'  # append if already exists
    else:
        append_write = 'w'  # make a new file if not

    runtimeResults = open('runtimes_InceptionV1_Clipper.txt', append_write)
