...
def getResnet50():
  with gfile.FastGFile("frozen_inception_v1.pb",'rb') as f:
    graph_def = tf.GraphDef()
    graph_def.ParseFromString(f.read())
  return graph_def


def timeGraph(gdef,batch_size=128,image_folder='images',nvidiasmi='output.out', latencyF = 'latency.txt', jobID='1'):
  tf.logging.info("Starting execution")
  gpu_options = tf.GPUOptions(per_process_gpu_memory_fraction=0.075)
...
##    dummy_input = np.random.random_sample((batch_size,224,224,3))
  ...
  with g.as_default():
    ...
    out = tf.import_graph_def(
      ...
      return_elements=[ "InceptionV1/Logits/Predictions/Softmax"]
    )
    out = out[0].outputs[0]
    print("\n\n image out",out,"\n\n")
    outlist.append(out)
  ...
  with tf.Session(graph=g,config=tf.ConfigProto(gpu_options=gpu_options)) as sess:
      ...
      nvidiasmiCommand = "nohup nvidia-smi --query-gpu=index,timestamp,temperature.gpu,utilization.gpu,utilization.memory,memory.used,memory.free,memory.total,power.draw,clocks.sm,clocks.mem,clocks.gr --format=csv -l 1 -f " + nvidiasmi + " &"
      ...
      for k in range(num_iters):
        ...
        if os.path.exists(jobID + '_terminate.txt'):  # terminate the job
            break
        ...
        runtimeResults.flush()


        #printing lables
        printLables = 0
        if printLables == 1:
            if os.path.exists('resultLables_InceptionV1.txt'):
                append_write = 'a' # append if already exists
            else:
                append_write = 'w' # make a new file if not
            #
            highscore = open('resultLables_InceptionV1.txt',append_write)
      ...
      # if os.path.exists('runtimes_InceptionV1.txt'):
      #     append_write = 'a' # append if already exists
      # else:
      #     append_write = 'w' # make a new file if not
      #
      # runtimeResults = open('runtimes_InceptionV1.txt',append_write)
...
if "__main__" in __name__:
  ...
  P.add_argument('--job_id', type=str, default='1')
  ...
  if f.native:
    startTime = time.time()
    timings,comp,mdstats=timeGraph(getResnet50(),f.batch_size,f.image_folder,f.nvidiasmi_outputFile,f.latencyFile, f.job_id)

    endTime2 = time.time()

    if os.path.exists('runtimes_InceptionV1_MT.txt'):
        append_write = 'a'  # append if already exists
    else:
        append_write = 'w'  # make a new file if not

    runtimeResults = open('runtimes_InceptionV1_MT.txt', append_write)
    runtimeResults.write('Total Execution Time, ' + str(endTime2 - startTime))
    runtimeResults.write("\n")
    runtimeResults.close()

    finished = open(f.job_id + "_finished.txt", 'w')
