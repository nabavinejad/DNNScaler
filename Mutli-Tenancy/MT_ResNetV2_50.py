...
##    dummy_input = np.random.random_sample((batch_size,224,224,3))
  ...
  with tf.Session(graph=g,config=tf.ConfigProto(gpu_options=gpu_options)) as sess:
      ...
      for k in range(num_iters):
        ...
        printLables = 0
        if printLables == 1:
            if os.path.exists('resultLables_InceptionV4.txt'):
                append_write = 'a' # append if already exists
            else:
                append_write = 'w' # make a new file if not
            #
            highscore = open('resultLables_InceptionV4.txt',append_write)
...
if "__main__" in __name__:
  ...
  if f.native:
    ...
    if os.path.exists('runtimes_ResNetV2_50_MT.txt'):
        append_write = 'a'  # append if already exists
    else:
        append_write = 'w'  # make a new file if not

    runtimeResults = open('runtimes_ResNetV2_50_MT.txt', append_write)
